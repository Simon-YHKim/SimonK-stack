...
- **`context-guardian`** — 이 파일이 관리하는 규칙의 source. 세션 시작 시 auto-heal. 4 모드 (prevention / monitoring / proactive-compact / recovery)
- **`session-context-tracker`** — 매 응답 후 세션 5선택지 (continue/rewind/clear/compact/subagent) 자가 점검. 긴 세션·context rot 대응
- **`html-default-output`** — 사람이 읽을 산출물 (스펙·리포트·PR 설명) 을 기본 HTML 로. 일회용 에디터 후보 식별
- **`commit`** — Conventional Commits 준수
- **`review`** — PR 사전 리뷰
- **`simon-tdd`** — 새 script/feature 추가 시 RED-GREEN-REFACTOR
- **`/ship`** (Gstack) — VERSION + CHANGELOG + push + PR

### Skill Routing — 신규 skill 등록 (2026-05-21)

| 트리거 | skill |
|---|---|
| 세션이 길어짐 / "rewind 할까" / "compact 해줘" | `session-context-tracker` |
| "HTML 로 만들어줘" / 스펙·리포트·PR 설명 산출 | `html-default-output` |
| 컨텍스트 70/80 % 도달 | `context-guardian` (proactive-compact 모드) |
| "에이전트 위임" / 중간 출력 불필요한 작업 | `agent-delegate` (Step 0 판정 질문) |
...
**SimonK Stack 의 모든 작업은 [Simon-LLM-Wiki](https://github.com/Simon-YHKim/Simon-LLM-Wiki) 를 _먼저_ 참고합니다.**

이는 instincts 와 보완 관계:
- `instincts` = 코딩 도메인 (mistakes, patterns, korean API, tool quirks)
- `Simon-LLM-Wiki` = 사용자 메타 인지 도메인 (작업 성향, 의사결정 패턴, 누적 결론)
...
```bash
cd ~/.claude/wiki/Simon-LLM-Wiki 2>/dev/null && git fetch --quiet origin && \
  [ "$(git rev-list HEAD..origin/main --count 2>/dev/null)" -gt 0 ] && \
  echo "[wiki] origin has new commits — git pull 권장"

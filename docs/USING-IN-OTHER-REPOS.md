...
이후 어느 프로젝트에서 Claude Code 를 열든 60+ skill 자동 사용 가능. 제거는 `~/.claude.bak-*` 백업에서 복원.
...
simon-stack 레포가 private 이거나, 네트워크 의존성 없이 self-contained 하고 싶을 때. **24개 skill 파일을 target 레포에 직접 copy**.
...
│   ├── settings.json
│   ├── skills/                   ← 24개 simon-stack skill 전체
│   │   ├── INDEX.md
...
| 웹, 타 레포, 일반 (권장) | §4 Vendor | 매 세션 (Gstack) | 28+ | 수동 |

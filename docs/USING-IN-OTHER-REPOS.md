...
└── ☁️ 웹 (claude.ai/code)
    │
    ├── 이 레포(SimonK-stack)에서 작업
    │   └── → §2. 자동 동작 (아무것도 안 해도 됨)
    │
    └── 다른 레포에서 simon-stack 을 쓰고 싶음
        │
        ├── SimonK-stack 레포가 public?
...
이후 어느 프로젝트에서 Claude Code 를 열든 100+ skill 자동 사용 가능. 제거는 `~/.claude.bak-*` 백업에서 복원.

**업데이트**: 새 버전이 나오면 `cd SimonK-stack && git pull && ./scripts/install.sh` 재실행 (idempotent, 안전).

---

## §2. Claude Code 웹에서 이 레포 (자동)

https://claude.ai/code → `Simon-YHKim/SimonK-stack` 열기 → **끝**.
...
**상태**: ✅ **이 레포가 public 으로 전환돼 즉시 사용 가능**

Claude Code 웹 sandbox 가 `git clone -b main https://github.com/Simon-YHKim/SimonK-stack` 을 auth 없이 성공.
...
/path/to/SimonK-stack/scripts/setup-repo.sh --mode bootstrap .

# Option B: curl 스트림
curl -sSL https://raw.githubusercontent.com/Simon-YHKim/SimonK-stack/main/scripts/setup-repo.sh | bash -s -- --mode bootstrap
```
...
simon-stack 레포가 private 이거나, 네트워크 의존성 없이 self-contained 하고 싶을 때. **100+ skill 파일을 target 레포에 직접 copy**.

### 설치

```bash
cd /path/to/SimonK-stack
...
│   ├── settings.json
│   ├── skills/                   ← 100+ simon-stack skill 전체
│   │   ├── INDEX.md
...
```bash
cd /path/to/SimonK-stack && git pull
cd /path/to/target-repo
...
| 장점 | 단점 |
|---|---|
| **네트워크 독립** — simon-stack repo private 여도 OK | Target 레포 size 증가 (~24 SKILL.md 파일) |
...
- **로컬 글로벌**: `cd SimonK-stack && git pull && ./scripts/install.sh`
...
| 웹, 타 레포, 일반 (권장) | §4 Vendor | 매 세션 (Gstack) | 100+ | 수동 |

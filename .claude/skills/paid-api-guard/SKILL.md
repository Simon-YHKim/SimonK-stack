---
name: paid-api-guard
description: 유료 API 6층 방어 — 결제·SMS·지도·금융·이메일 API 남용 방지 및 API 설계 리뷰. Defense-in-depth for paid third-party APIs (Stripe, Toss, Twilio, Maps, email) and API design review. 트리거 키워드 — 유료 API 보안, 결제 API, Stripe, Toss, Iamport, Twilio, 외부 API 키 보안, API 해킹 방지, 비용 남용, 결제 보안, SMS 보안, API 설계 리뷰.
...
- **브라우저 직접 호출 금지** — 항상 BFF (Backend-for-Frontend) 경유
...
- **금액 서버 재계산**: 클라이언트가 보낸 amount 절대 신뢰 금지
  ```ts

...
TARGETS = {
    ...
    "cowork_A_§0+§8": {
        "sections": [0, 8],
        "place": "Cowork → 상시 지시사항 (A안 — 프로필이 Cowork 에도 닿을 때)",
    },
    "cowork_B_§0-6+§8": {
        "sections": GLOBAL + [8],
        "place": "Cowork → 상시 지시사항 (B안 — 닿지 않을 때)",
    },
}
...
REFS_BASELINE = {
    ...
    "cowork_A_§0+§8": 6,        # §8 의 기존 결함 4 + §0 이 더한 §1·§5
    "cowork_B_§0-6+§8": 1,      # §8 → §7 (Q-260830-03)
}
...
def check_refs(text):
    ...
    참조하므로 Cowork **A안에서만** 같은 결함이 2건 늘어난다 — B안은 §1~§6 을 갖고
    있어 §0 의 참조가 전부 해소된다. 이 표가 A/B 선택의 정량 근거다.
    """
    total = 0
    over = 0
    for name, spec in TARGETS.items():

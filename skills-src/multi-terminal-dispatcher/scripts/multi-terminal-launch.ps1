# Canonical package entry for a guarded /vibe wave, not a window launcher.
# Invoke with pwsh -NoProfile -NonInteractive -File (do not dot-source).
# Default/DryRun previews logical state; the existing DB may be write-locked.
...
$helper = Join-Path $PSScriptRoot 'dispatch_wave.py'
